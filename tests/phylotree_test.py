...
# import phyling._config as _config
# import phyling.exception as exception
# import phyling.tree
# from phyling.tree.tree import MFA2Tree, MFA2TreeList
# from phyling.pipeline._outputprecheck import TreePrecheck
# from phyling.tree._libtree import determine_samples_and_seqtype
...
#         assert isinstance(m.load, MultipleSeqAlignment)
...
#     wrapper_pep = MFA2TreeList(files_pep, seqtype="pep")
#     wrapper_cds = MFA2TreeList(files_cds, seqtype="cds")
...
#     def test_init(self, wrapper: MFA2TreeList, seqtype: str):
...
#     def test_getitem(self, wrapper: MFA2TreeList, sorted_wrapper: MFA2TreeList):
#         assert len(wrapper) == 3
#         assert all(isinstance(x, MFA2Tree) for x in wrapper)
#         w_sub = sorted_wrapper[0:2]
#         assert isinstance(w_sub, MFA2TreeList)
#         assert len(w_sub) == 2
...
#     def test_getitem_attributeerror(self, wrapper: MFA2TreeList, built_wrapper: MFA2TreeList):
#         with pytest.raises(AttributeError, match=r"build\(\) need to be run first"):
#             wrapper[0:2]
#         with pytest.raises(AttributeError, match=r"compute_toverr\(\) need to be run first"):
#             built_wrapper[0:2]
...
#     def test_sort(self, wrapper: MFA2TreeList):
#         w = deepcopy(wrapper)
#         w.compute_toverr()
#         w.sort()
...
#     @pytest.mark.parametrize("wrapper, built_wrapper", ((wrapper_pep, built_pep), (wrapper_cds, built_cds)))
#     def test_sort_attributeerror(self, wrapper: MFA2TreeList, built_wrapper: MFA2TreeList):
#         with pytest.raises(AttributeError, match=r"build\(\) need to be run first"):
#             wrapper.sort()
#         with pytest.raises(AttributeError, match=r"compute_toverr\(\) need to be run first"):
#             built_wrapper.sort()
...
#     def test_build(self, wrapper: MFA2TreeList, method: str):
#         w = deepcopy(wrapper)
#         w.build(method)
#         assert w.method == method

#     @pytest.mark.parametrize("wrapper", (built_pep, built_cds))
#     def test_compute_toverr(self, wrapper: MFA2TreeList):
#         w = deepcopy(wrapper)
#         w.compute_toverr()
#         assert all(isinstance(x.toverr, float) for x in w)

#     @pytest.mark.parametrize("wrapper", (sorted_pep, sorted_cds))
#     def test_get_consensus_tree(self, wrapper: MFA2TreeList):
#         w = deepcopy(wrapper)
...
#         ),
#     )
#     def test_concat(self, tmp_path: Path, wrapper: MFA2TreeList, samples: tuple, partition: str):
#         w = deepcopy(wrapper)
...
#             "inputs": "fakechecksum",
#         }
#         r, _ = TreePrecheck.precheck(params)
#         assert r == 0
...
#         fake_input.touch()
#         TreePrecheck.save_checkpoint(prev_params, MFA2TreeList([fake_input], seqtype="pep"))
#         cur_params = {
#             "method": method2,
#             "top_n_toverr": 18,
#             "concat": True,
...
#             "inputs": "fakechecksum",
#         }
#         r, _ = TreePrecheck.precheck((cur_params))
#         assert r == 3
...
#         fake_input = tmp_path / "input"
#         fake_input.touch()
#         TreePrecheck.save_checkpoint(prev_params.copy(), MFA2TreeList([fake_input], seqtype="pep"))
#         cur_params = {
#             "method": method[1],
#             "top_n_toverr": top_n[1],
#             "concat": concat[1],
...
#                 TreePrecheck.precheck(cur_params)
#         else:
#             r, _ = TreePrecheck.precheck(cur_params)
#             assert r == 0

#     def test_load_checkpoint(self):
#         TreePrecheck.setup(folder=Path("tests/data/tree"))
#         params, wrapper = TreePrecheck.load_checkpoint()
#         assert isinstance(params, dict)
#         assert isinstance(wrapper, MFA2TreeList)
...
#             "inputs": "fakechecksum",
#         }
#         wrapper = MFA2TreeList(["tests/data/pep_align/10at10240.aa.mfa"], seqtype="pep")
#         TreePrecheck.save_checkpoint(params, wrapper)
#         a, b = TreePrecheck.load_checkpoint()
#         assert len(a) == 6
#         assert b == wrapper
...
#         TreePrecheck.setup(folder=tmp_path, method=method, concat=concat, partition=partition, figure=figure)
#         _, wrapper = TreePrecheck.load_checkpoint()
#         TreePrecheck.output_results(wrapper[0].tree)

#         with open(tmp_path / "final_tree.nw") as f:
#             content = f.read()
#         strategy = "concatenate" if concat else "consensus"
#         assert f"Final tree is built using {phyling.tree.METHODS[method]} with {strategy} strategy" in content
